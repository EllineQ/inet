...
void SerializerBase::serializePacket(const cPacket *pkt, Buffer &b, Context& c)
{
    ...
    if (!b.hasError() && (b.getPos() - startPos != pkt->getByteLength()))
        throw cRuntimeError("%s serializer error: packet %s (%s) length is %ld but serialized length is %ld", getClassName(), pkt->getName(), pkt->getClassName(), pkt->getByteLength(), b.getPos() - startPos);
}
...
cPacket *SerializerBase::deserializePacket(const Buffer &b, Context& context)
{
    ...
    if (!pkt->hasBitError() && !b.hasError() && (b.getPos() - startPos != pkt->getByteLength())) {
        ...
        throw cRuntimeError("%s deserializer error: packet %s (%s) length is %ld but deserialized length is %ld (encapsulated packet is %s)", getClassName(), pkt->getName(), pkt->getClassName(), pkt->getByteLength(), b.getPos() - startPos, encclass);
    }
    if (b.hasError())
